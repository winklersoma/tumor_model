# In this file, the initial distributions follow uniform, and the sampled times are constant.
...
def logdelay():
    ...
    for i in range(repeat):
        M = m0
        R = np.random.uniform(low=0, high=(2 * 10 * 1.028), size=r0)  # np.random.gamma(k_r0, cc_r0, r0)
        Y = np.random.uniform(low=0, high=(2*10*0.387), size=y0)  # random.gamma(k_y0, cc_y0, y0)
        G = np.random.uniform(low=0, high=(2*10*1.285), size=g0)  # random.gamma(k_g0, cc_g0, g0)
        ...
        while t < 75 and np.any([0 < len(R), 0 < len(Y), 0 < len(G), M < K]):  # 47.75
            ...
            if min_tau == tau1:  # Ha a legrövidebb idő R-beli sejthez tartozik
                ...
                time_in_Y = k_y * cc_y  # a sejtciklus Y-ben töltött hossza (jöhet eloszlásból)
                ...
                time_in_G = k_g * cc_g  # a sejtciklus G-ben töltött hossza (jöhet eloszlásból)
                ...
                Y -= min_tau
                G -= min_tau
                time_in_R = k_r * cc_r  # a sejtciklus hossza
