...
def logdelay(drug_0=3, k_c=(1/3, 1/3, 1/3), delta=1, protocol=[1, 48, 72, 2000]):
    r_p = 50
    k_r = 10
    k_y = 10
    k_g = 10
    ...
    end_time = 120  # protocol[-1] + 24 if protocol[-1] != 0 else 100
    time_step = 0.05
    drug_step = 0.05
    Data = []
...
t1, r1, y1, g1, m1, d1 = logdelay(k_c=(0, 0, 1))

fig, ax = plt.subplots(nrows=1, ncols=2, sharey=True)


ax[0].set_ylabel('Nr. of cells', color="black")
ax[0].plot(t, m + r, c='red', linewidth=1)
ax[0].plot(t, y, c='orange', linewidth=1)
ax[0].plot(t, g, c='green', linewidth=1)
ax[0].plot(t, m + r + y + g, c='black', linewidth=2, label='Total nr. of cells')
ax[0].tick_params(axis='y', labelcolor="black")
ax[0].set_xlabel('Time')
if any(d) != 0 or 0.0:
    ax1 = ax[0].twinx()  # instantiate a second axes that shares the same x-ax1
    # ax2.set_ylim(bottom=0)
    # ax2.margins(y = 0.05)
    ax1.set_ylabel('Drug concentration, $\\frac{mg}{l}$', color="blue")
    ax1.step(t, d, "blue", where='pre', alpha=0.4)
    ax1.tick_params(axis='y', labelcolor="blue")
ax[1].plot(t1, m1 + r1 + y1 + g1, c='black', linewidth=2, label='Total nr. of cells')
ax[1].plot(t, m + r, c='red', linewidth=1)
ax[1].plot(t, y, c='orange', linewidth=1)
ax[1].plot(t, g, c='green', linewidth=1)
ax[1].tick_params(axis='y', labelcolor="black")
ax[1].set_xlabel('Time')
if any(d) != 0 or 0.0:
    ax2 = ax[1].twinx()  # instantiate a second axes that shares the same x-ax1
    # ax2.set_ylim(bottom=0)
    # ax2.margins(y = 0.05)
    ax2.set_ylabel('Drug concentration, $\\frac{mg}{l}$', color="blue")
    ax2.step(t, d, "blue", where='pre', alpha=0.4)
    ax2.tick_params(axis='y', labelcolor="blue")
ax[0].set_title('K_c = (1/3, 1/3, 1/3)')
ax[1].set_title('K_c = (0, 0, 1)')
fig.tight_layout()  # otherwise the right y-label is slightly clipped
plt.savefig('two_chemos', dpi=300)
plt.show()
