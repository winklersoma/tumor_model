...
def logdelay(drug_0=2.3869, k_c=(0, 0.9, 0), delta=0.5, protocol=[0, 24, 48, 72, 96], half_life=3):
    ...
    g0 = 500
    repeat = 1
    K = 1000000  # meg kellett emelni, mert elérte. (K - 2 * M * (len(R) + len(Y) + len(G))) < 0!!
    end_time = 168
    time_step = 0.05
    drug_step = 0.05
    Data = []
    ...
    for i in range(repeat):
        ...
        M = m0
        R = np.random.uniform(0, k_r*cc_r, r0)
        Y = np.random.uniform(0, k_y*cc_y, y0)
        G = np.random.uniform(0, k_g*cc_g, g0)
...
t, r, y, g, m, d = logdelay(protocol=list(np.linspace(0,120,20)))
