...
def logdelay():
    ...
    for i in range(repeat):
        M = m0
        R = np.random.gamma(shape=5, scale=1, size=r0)
        Y = np.random.gamma(shape=5, scale=1, size=y0)
        G = np.random.gamma(shape=5, scale=1, size=g0)

        t = 0
        t_stat = [0]
        R_stat, Y_stat, G_stat, M_stat = [len(R)], [len(Y)], [len(G)], [M]

        while t < 72 and np.any([0 < len(R), 0 < len(Y), 0 < len(G), M < K]):  # 47.75
...
ax1.set_ylabel('Number of cells', color="black")
ax1.plot(t, m, c="purple",linestyle='dotted', linewidth=1.5, label='Nr. of cells in M')
...
ax1.set_xlabel('Time')
plt.legend()
plt.savefig('decreased_r.png', dpi=300)
plt.show()
